...
const MedicineDetails = () => {
  ...
  const handleStripChange = async (id, newStripValue) => {
    try {
      const parsedStrip = parseInt(newStripValue, 10);
      if (isNaN(parsedStrip)) return;

      await api.put(`/medicine/${id}`, { strip: parsedStrip });
      setMedicines(medicines.map(medicine =>
        medicine._id === id ? { ...medicine, strip: parsedStrip } : medicine
      ));
    } catch (err) {
      console.error('Failed to update medicine strip count', err);
    }
