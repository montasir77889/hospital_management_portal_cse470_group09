 ...
 * @desc    Update medicine stock (increase or decrease)
 * @body    { operation: "increase" | "decrease", strip: number }
 */
router.put('/:id', async (req, res) => {
  const { operation, strip } = req.body;

  if (!operation || typeof strip !== 'number') {
    return res.status(400).json({
      message: 'Invalid request body. Please provide operation and strip count.',
    });
  }
  ...
  try {
    ...
    if (operation === 'increase') {
      ...
      medicine.strip -= strip;
      if (medicine.strip < 0) medicine.strip = 0;
    } else {
